...
int main(){
   ...
   int i,auxiliar,suma = 0;
   std::string digito;
   for( i = 0 ; i < numDigitos ; i++){//desde aca
    digito = codigo[i];
    auxilia r= stoi(digito);
   ...
   if(suma % 10 == 0){
    std::cout<<"su tarjeta\t"<<codigo<<"\tsi cumple el algoritmo de luhn";
   }else{
    std::cout<<"su tarjeta\t"<<codigo<<"\tno cumple el algoritmo de luhn";
   }
   return 0;
}